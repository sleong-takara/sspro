...
path0=folder1+r"20260323_Hammerhead SSPro rerun_S2016" #v2 hammer raw
path1=folder+"241209_SSPro_Beta2" #v1 raw
path2=folder1+r"20260323_Hammerhead SSPro rerun_S2016_100K" #v2 hammer 50K
path3=folder+"241209_SSPro_Beta2_100K" #v1 50K
...
specific_order = [shasta+'_Raw',cx+'_Raw',shasta+'_100K',cx+'_100K'] #v2 raw, v1 raw, v2 50k, v1 50k

sample_name=['PBMC','Sample'] #v2,v1
...
sspro_spec1_df = func.sspro_spec1_defunct(df0)[0] #v2 - shasta
# sspro_spec1_df = func.sspro_spec1_defunct(df1)[0] #v1 - cx
...
cx_spec2=df1[(df1["Well_Type"] == "PBMC")| (df1["Well_Type"] == "sample") | (df1["Well_Type"] == "Sample")] #CX1018
...
#%% REMOVE MISSING ROWS; DO NOT RUN!!!!!!!

# row_low_reads = list_of_df[1].groupby(['Row'])['Barcoded_Reads'].median().sort_values(ascending=True)
# list_of_df[1] = list_of_df[1][~(list_of_df[1]['Row'].isin(row_low_reads)) & ~(list_of_df[1]['Barcoded_Reads'] < 1000)]

# list_of_df[0] = list_of_df[0][(list_of_df[0]['Col'] != 37) ] #shasta: remove col 37


# list_of_df[0] = list_of_df[0][~(list_of_df[0]['Barcoded_Reads']<1000)]
# list_of_df[0] = list_of_df[0][~((list_of_df[0]['Row'] < 6) & (list_of_df[0]['Barcoded_Reads'] < 5000))] #shasta: remove rows 0-6 with reads <5000
...
#group then sum USE THIS ONE 2024
...
print(exonReads.sort_index(ascending=False).round(2))
...
# Print the resulting ribosomal + mitochondrial fractions for each group
print(riboMitoSum.sort_index(ascending=False).round(2))
...
func.plot_table_from_df(sscomp_spec1_df,'Reads',base_width=2.75)
...
func.plot(test, ['Barcoded_Reads'],'Sample',[specific_order[0],specific_order[1]], base_width=3.5, base_height=4.5)

test=df[df['Sample'].isin(specific_order[2::])]


func.plot(df, ['No_of_Genes'], 'Sample',specific_order, base_width=8,base_height=5)

test=df[df['Sample'].isin([specific_order[2],specific_order[3]])]

# func.plot(df, ['No_of_Genes'], 'Sample',[specific_order[2],specific_order[3]], base_width=8,base_height=4)
...
func.plot_by_fraction(df, ['Total_Exon_Reads', 'Total_Intron_Reads'], 'Barcoded_Reads', 'Sample',specific_order, base_width=9)
